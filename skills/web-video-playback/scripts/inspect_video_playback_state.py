...
def _loads_json_tolerant(source: str) -> Any:
    """解析模型/API JSON，并只修复不合法的反斜杠转义。"""
    try:
    ...
    while index < len(source):
        ...
        valid = following in {'"', "\\", "/", "b", "f", "n", "r", "t"}
        if following == "u":
            digits = source[index + 2:index + 6]
            valid = len(digits) == 4 and all(
                digit in "0123456789abcdefABCDEF" for digit in digits
            )
        repaired.append("\\" if valid else "\\\\")
        index += 1
...
def _extract_json(text: str) -> dict[str, Any]:
    ...
    # 少数视觉响应会直接输出 Windows 路径或类似 ``\video`` 的内容，
    # 形成 JSON 不允许的 \v、\p 等转义。
    value = _loads_json_tolerant(match.group(0))
    ...
    for attempt in range(3):
        try:
            with urllib.request.urlopen(request, timeout=90) as response:
                # DeepSeek 偶尔会把 content 中的反斜杠原样放入外层响应；先按
                # UTF-8 读取，再使用与内层结果相同的有限容错解析。
                response_text = response.read().decode("utf-8")
                result = _loads_json_tolerant(response_text)
            break
        except urllib.error.HTTPError as exc:
            body = exc.read().decode("utf-8", errors="replace")
            if exc.code not in {408, 429, 500, 502, 503, 504} or attempt >= 2:
                raise RuntimeError(
                    f"视觉 API 请求失败（HTTP {exc.code}）：{body}"
                ) from exc
        except (urllib.error.URLError, ConnectionResetError,
                ConnectionAbortedError, TimeoutError, OSError) as exc:
            reason = getattr(exc, "reason", exc)
            winerror = getattr(reason, "winerror", getattr(exc, "winerror", None))
            retryable = (
                isinstance(reason, (ConnectionResetError, ConnectionAbortedError, TimeoutError))
                or winerror in {10053, 10054, 10060, 11001}
                or any(token in str(reason).lower()
                       for token in ("connection reset", "timed out", "10054"))
            )
            if not retryable or attempt >= 2:
                raise RuntimeError(f"视觉 API 连接失败：{reason}") from exc
        time.sleep(attempt + 1)

    if result is None:
        raise RuntimeError("视觉 API 重试后仍未返回结果")
...
2. 页面标题或播放器附近标题是否与目标视频相符（只用于记录，不影响正在播放的判定）。
3. 是否显示播放器、播放按钮、暂停按钮或加载动画。
4. 前后两张图中，播放器画面是否变化，进度条或时间是否前进。
5. 是否有登录、验证码、付费、年龄确认、地区限制等阻断界面。
6. 是否被其他窗口遮挡，导致无法看清网站或播放器。

广告、片头或正片只要位于视频网站的可见播放器内并且确实正在播放，都算播放成功。
不要根据页面背景动画、鼠标闪烁或播放器之外的区域变化判断视频正在播放。只返回 JSON：
{{
...
def inspect_video_playback_state(
    ...
    *,
    ...
    if indicators["window_obscured"]:
        ...
        state = "not_visible"
    # 用户只要求进入播放页后有视频实际播放。广告、片头和正片使用同一完成条件；
    # 因此先检查动态播放证据，标题不匹配不能覆盖已经确认的 playing。
