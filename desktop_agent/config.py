...
def default_skills_dir() -> Path:
    """返回源码目录或已安装数据目录中的内置 Skill 路径。"""
    source_skills = Path(__file__).resolve().parent.parent / "skills"
    if (source_skills / "config.json").is_file():
        return source_skills
    installed_skills = (
        Path(sysconfig.get_path("data")) / "share" / "desktop-ai-agent" / "skills"
    )
    if (installed_skills / "config.json").is_file():
        return installed_skills
    # 保留清晰的缺失路径，SkillRegistry 会把它当作没有可用 Skill；调用者也能
    # 从错误信息中看到安装包缺少了哪一部分。
    return installed_skills
...
class AgentConfig:
    ...
    skills_dir: Path = field(default_factory=default_skills_dir)
    grid_rows: int = 10
    grid_columns: int = 10
    ...
    tool_call_limits: dict[str, int] = field(
        default_factory=lambda: {
            ...
            "inspect_music_playback": 6,
            "resume_music_playback": 3,
            "find_desktop_file": 3,
        }
    )
