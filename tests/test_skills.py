...
class SkillRegistryTests(unittest.TestCase):
    ...
    def test_default_skills_path_is_independent_of_working_directory(self) -> None:
        original = Path.cwd()
        with TemporaryDirectory() as directory:
            os.chdir(directory)
            try:
                skills_dir = AgentConfig().skills_dir
            finally:
                os.chdir(original)
        self.assertTrue((skills_dir / "config.json").is_file())
        self.assertEqual(skills_dir, Path(__file__).parents[1] / "skills")

    def test_prepares_paper_translation_variables(self) -> None:
        cases = {
            "翻译 GROOT 论文": "GROOT",
        ...
        for task, expected in cases.items():
            with self.subTest(task=task):
                match = self.registry.prepare("chatgpt-paper-abstract", task)
    ...
    def test_prepares_video_playback_variables(self) -> None:
        cases = (
            (
        ...
        for task, platform, url, query in cases:
            with self.subTest(task=task):
                match = self.registry.prepare("web-video-playback", task)
    ...
    def test_skill_task_requires_confirmation(self) -> None:
        match = self.registry.prepare("chatgpt-paper-abstract", "翻译 GROOT 论文")
        request = TaskRequest(
            kind="skill",
            payload={
                "task": "翻译 GROOT 论文",
