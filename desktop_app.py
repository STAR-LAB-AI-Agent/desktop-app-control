...
class DesktopAgentWindow:
    def __init__(self) -> None:
        ...
        # 根据当前 Windows 字体和缩放计算宽度，避免终止按钮被截断。
        self.root.update_idletasks()
        required_width = max(810, self.settings.winfo_reqwidth() + 24)
        required_height = max(145, self.root.winfo_reqheight())
        self.root.geometry(f"{required_width}x{required_height}+40+40")
        self.root.minsize(required_width, required_height)

        self._busy = False
        self._active_agent: DesktopAgent | None = None
        self._active_task_id: str | None = None
        self._escape_listener: EscapeStopListener | None = None
    ...
    def _submit(self, _event=None) -> str:
        ...
        try:
            ...
            messagebox.showerror("任务启动失败", str(exc), parent=self.root)
            return "break"
        ...
        self._escape_listener = EscapeStopListener(
            lambda: agent.cancel(task.id),
        ).start()
        self.entry.configure(state="disabled")
        self._set_settings_state("disabled")
        self.stop_button.configure(state="normal")
        self.root.title("Desktop Agent — 执行中")
        # 先取消置顶并最小化，等待窗口动画和审核对话框消失后再截图。
