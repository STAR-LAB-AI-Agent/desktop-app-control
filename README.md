...
一个使用视觉模型定位界面元素、使用 PyAutoGUI 执行动作的桌面智能体最小可用版本。
...
当前只保留两条执行主线：由视觉模型逐步选择原子工具的通用工作流，以及按任务语义
自动路由的场景技能。网页搜索属于通用任务，不再维护百度专用工作流。
...
翻译桌面论文的摘要（Abstract）以及第 1 节到第 5 节（Section 1 至 Section 5）
（会自动命中 `chatgpt-paper-abstract` 技能）：

```powershell
python main.py run "翻译 GROOT 论文"
```

该技能会先查找桌面上名称唯一匹配的 PDF，再打开 ChatGPT 桌面版、新建聊天、
选择 Work、上传精确匹配的文件，然后依次请求翻译摘要以及第 1 节到第 5 节。
每段提交后都会等待生成完成；没有匹配或存在多个匹配时会停止，不会猜测文件；
若遇到登录或身份验证，也会停止并交给用户处理。

查找视频网站并播放指定视频（会命中 `web-video-playback` 技能）：

```powershell
python main.py run "在哔哩哔哩搜索并播放 Python 教程视频"
```

该技能会优先进入用户指定的视频网站；未指定平台时会从搜索结果中选择可靠的视频站点。
播放状态检查器会间隔截取两张屏幕，对比播放器画面和进度条，只有确认目标视频正在播放
才会结束任务。登录、验证码、付费和地区限制交给用户处理。
...
改用更具体的目标描述、键盘操作或等待。搜索输入后优先使用回车键；确认没有结果时，
才使用带网格辅助线的鼠标点击作为回退。
...
- `desktop_agent/skills.py`：发现、加载和路由项目内的场景技能。
- `desktop_agent/audit.py`：执行前任务审核。
- `desktop_agent/runtime.py`：审批、状态流转、异常处理和日志。
- `desktop_agent/cli.py`：启动入口及人工审批交互。
- `skills/<skill-name>/SKILL.md`：场景技能的长程规划、恢复策略和安全边界。
- `skills/<skill-name>/skill.py`：任务匹配、变量提取、审核配置和可选执行前预检。
- `skills/<skill-name>/scripts/`：跨运行时的确定性脚本；可由 nanobot 直接执行，也可通过清单适配成本项目工具。
- `skills/config.json`：统一配置技能的启用状态和路由优先级。
- `skills/manage.py`：列出、检查、启用、禁用技能和修改优先级的管理入口。
- `tests/script_cli/`：不依赖主智能体循环的脚本与命令行接口测试。
- `example/`：环境变量设置、通用任务和视频播放演示脚本。
- `docs/SKILL_AND_ROUTING_GUIDE.md`：新增技能和配置路由的中文教程与模板。

日志统一写入 `logs/<时间_任务简介>/`。新增固定工作流时，实现一个 `Workflow` 并注册到
`DesktopAgent`；新增场景技能时只需要增加技能文件夹，不需要修改主工作流或审核器。

`main.py` 通过 `AgentConfig` 向智能体传入 API、模型、信任模式、定位阈值、
最大步数、动作间隔和日志路径。新增工具时创建一个 `ToolSpec`，再调用
`agent.register_tool(...)`；新增任务时调用 `agent.register_workflow(...)`。

当任务需要启动尚未显示的软件时，智能体必须调用原子工具
`open_app_via_windows_search`。工具内部一次完成 `Win+S`、覆盖旧查询、输入软件名和
按回车键；模型不会在中途根据搜索历史、推荐项或最近使用记录做点击决策。工具返回后，
智能体才根据新截图确认窗口是否出现。默认名称映射配置在
`AgentConfig.application_search_names`，例如浏览器使用 `Edge`、音乐软件使用“音乐”、
翻译软件使用 `ChatGPT`；可继续添加自己的软件别名。

在浏览器或应用内搜索时，智能体聚焦搜索框后调用 `submit_search_query`，原子完成覆盖
旧文本、输入查询和按回车键。自动补全、历史记录、曾搜索内容和推荐词不会成为下一轮
...
python skills/manage.py disable web-video-playback
python skills/manage.py enable web-video-playback
python skills/manage.py priority web-video-playback 90
```

新技能目录即使没有写入 `skills/config.json` 也会被自动发现，并默认启用、优先级为 0；
需要统一控制时再把它加入配置文件。多个技能同时命中时，优先级高者生效；最高优先级
相同则停止并报告路由冲突。
...
终止控制与审核仍复用智能体主链路。这样不会为每个场景复制一套智能体，也能保证
未命中的普通任务继续走通用工作流。

`SKILL.md + scripts/` 遵循智能体技能的可移植结构。nanobot 等运行时可按技能
中的相对路径直接运行脚本；本项目读取相邻的 `*.tool.json`，用
`agent-skill-script/v1` JSON 协议调用同一脚本，不维护第二份 Python 工具实现。

从零配置自己的技能，请直接阅读
[技能与路由配置指南](docs/SKILL_AND_ROUTING_GUIDE.md)。
